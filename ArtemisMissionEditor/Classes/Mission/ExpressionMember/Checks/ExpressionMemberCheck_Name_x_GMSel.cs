...
namespace ArtemisMissionEditor
{
	...
	public sealed class ExpressionMemberCheck_Name_x_GMSel : ExpressionMemberCheck
	{
		...
		public ExpressionMemberCheck_Name_x_GMSel(string EMVDname = "name")
			: base("", EMVD.GetItem("check_name/gmsel"))
		{
			List<ExpressionMember> eML;
			
			eML = this.Add("with name"); //_choices[0]
			eML.Add(new ExpressionMember("<>", EMVD.GetItem(EMVDname)	, "name"));

			eML = this.Add("selected by GM"); //_choices[1]
			eML.Add(new ExpressionMember("", EMVD.GetItem("use_gm"), "use_gm_selection"));
		}
