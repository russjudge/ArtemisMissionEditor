...
namespace ArtemisMissionEditor
{
    public sealed class TreeViewEx : TreeView
    {
		...
		public TreeNode FindNode_private_RecursivelyFind(TreeNode node, Func<TreeNode, bool> searcher)
		{
			TreeNode result = null;

			foreach (TreeNode cnode in node.Nodes)
				if ((result = FindNode_private_RecursivelyFind(cnode, searcher)) != null)
					return result;
        ...
        public TreeNode FindNode(Func<TreeNode, bool> searcher)
        {
			...
			foreach (TreeNode node in Nodes)
				if ((result = FindNode_private_RecursivelyFind(node, searcher)) != null)
					return result;
			return result;
        }
		...
		public bool NodeIsInsideNode(TreeNode what, TreeNode where)
		{
			...
			foreach (TreeNode node in where.Nodes)
				if (FindNode_private_RecursivelyFind(node, (TreeNode x) => x == what) != null)
					return true;
			return false;
		}
