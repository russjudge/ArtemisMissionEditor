...
namespace ArtemisMissionEditor.SpaceMap
{
    ...
    public enum SphereBox
    {
        Sphere,
        Box
    }

    public abstract class MapObjectSpecial
    {
    ...
    public sealed class SpecialMapObject_DestroyCircle : MapObjectSpecial
    {
	...
	public sealed class SpecialMapObject_PointTarget : MapObjectSpecial
	{
		[DisplayName("Statement"), Description("Specifies the statement being edited"), Category("ID")]
    ...
    public sealed class SpecialMapObject_BoxSphere : MapObjectSpecial
    {
        public static readonly int _minRadius = 0;
        public static readonly int _maxRadius = 100000;

        [DisplayName("Statement"), Description("Specifies the statement being edited"), Category("ID")]
        public string Statement { get { return "If object is " + (InOut == InsideOutside.Inside ? "inside " : "outside ") + (SphereBox == SphereBox.Sphere ? "sphere" : "box"); } }
        ...
        public InsideOutside InOut
        {
            get
            {
                if (_nodeName == "if_inside_sphere" || _nodeName == "if_inside_box")
                    return InsideOutside.Inside;
                else
                    return InsideOutside.Outside;
            }
            set
            {
                if (value == InsideOutside.Inside)
        ...
        public SphereBox SphereBox
        {
            ...
            {
                if (_nodeName == "if_outside_box" || _nodeName == "if_inside_box")
                    return SphereBox.Box;
                else
                    return SphereBox.Sphere;
            }
            set
            {
                if (value == SphereBox.Box)
        ...
        public override bool ExecuteWheelAction(int delta, Keys modifierKeys, int x, int y, int pixelsInOne)
        {
            ...
            {
                if (InOut == InsideOutside.Inside)
                    InOut = InsideOutside.Outside;
                else
                    InOut = InsideOutside.Inside;
                return true;
            }
            return false;
        }
        ...
        public override bool ExecuteMouseAction(MouseButtons buttons, Keys modifierKeys, int x, int y, int pixelsInOne)
        {
            ...
            {
                if (SphereBox == SphereBox.Sphere)
                    SphereBox = SphereBox.Box;
                else
                    SphereBox = SphereBox.Sphere;
            }

            if (buttons == MouseButtons.Left && ((modifierKeys & Keys.Alt) == Keys.Alt))
            {
                if (SphereBox == SphereBox.Box)
                {
                    _coordinatesEnd.X_Scr = x;
                    _coordinatesEnd.Z_Scr = y;
                    return true;
